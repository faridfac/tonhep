...
(async () => {
    const dataAkun = fs.readFileSync('wallet.txt', 'utf-8');
    const akun = dataAkun.split('\n');
    console.log(`[ ${moment().format("HH:mm:ss")} ] `, `${chalk.blue(`Total account : ${akun.length}`)}`);
    console.log('');
    for (let i = 0; i < akun.length; i++) {
        try {
            const split = akun[i].split("|");
            const privateKey = split[0];
            const address = split[1];
            console.log(`[ ${moment().format("HH:mm:ss")} ] `, `${chalk.green(`Using wallet : ${address}`)}`);
            const claim1 = await claimToken(address, 'HTE')
